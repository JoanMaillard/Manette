...
#define MASTER 0 // On change cette valeur à 1 si on est sur le Master ou à 0 si on est sur le Slave !
...
void setup() {
  ...
  // Attention, cette liste doit être exactement la même sur le Master ! (Dans le même ordre avec les mêmes noms de packet id)
...
void loop() {
  ...
  if (communicationProtocol.isAvailable() > 0)
  {
    ...
    communicationProtocol.addData("packetID1", "Bonjour");
    ...
    communicationProtocol.addData("packetID2", "Aurevoir");
