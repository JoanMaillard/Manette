...
char to_send_bouton5[2] = {bouton5, 0};
// [0] = bouton5 // [1] = the state of the button -> 2 if pressed, else 1
#else
...
//[8] = state of bouton 5 -> 2 if pressed, else 1
#endif
}
...
void bouton_pressed() {
  ...
  if (etat_bouton_4 || etat_bouton_3 || etat_bouton_2 || etat_bouton_1 || etat_bouton_5) {
    if (!etat_change) {
      change = true;
    }
    etat_change = true;
...
void check_joystick() {
  ...
  if (val_joystick[0] != val_joystick_base[0]) {
    val_joystick[0] = map(val_joystick[0], 0, 1023, 100, 999);
#if TRAME
  ...
  if (val_joystick[1] != val_joystick_base[1]) {
    val_joystick[1] = map(val_joystick[1], 48, 970, 100, 999);
#if TRAME
...
/*
  Serial.print("X: ");
  Serial.print(val_joystick_base[0]);
  Serial.print(" Y: ");
  Serial.println(val_joystick_base[1]);
  */
}
...
void check_change() {
  ...
  if (change_bouton5) {
    ...
    change_bouton5 = false;
    bus.send_packet(44, to_send_bouton5, 2);
  }
#else
  if (change) {
